...
// MQTT Topic
const char* topic_data = "bitanicv2/data";
const char* topic_device_statuts = "bitanicv2/device_status";
const char* subscribe_panel = "bitanicv2/BT01";
...
void deviceStatus() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("arduinoClient")) {
      ...
      client.publish(topic_device_statuts, "Device Online");
      // ... and resubscribe
...
void publishData() {
  ...
  client.publish(topic_data, json);
}
...
void sensorUpdate()
{
  ...
  if (millis() - sensorStartTime >= 5000)
  {
    ...
    soilMoisture1Value = digitalRead(soilMoisture1);
    delay(1000);
    soilMoisture2Value = digitalRead(soilMoisture2);
    sensorStartTime = 0;
  }
}
...
void loop()
{
  ...
  // mulai milis untuk menampilkan data pada LCD
  if (startTime == 0)
  {
    startTime = millis();
  }
  ...
  // print subscribe data to serial monitor
  OnDay = 0;
